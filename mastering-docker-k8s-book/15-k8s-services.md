...
<!-- VISUAL: Clients → Service VIP → three Pod IPs; Pod replace swaps backend without changing VIP -->
...
<!-- VISUAL: Client Pod on node A — Cluster policy may hop to node B; Local/PreferClose stays on node A or same zone -->
...
| NodePort | Lab access / on-prem entry without LB controller |
| LoadBalancer | Cloud or MetalLB external VIP |
| ExternalName | DNS alias to external hostname |
| Headless | Direct Pod DNS (StatefulSet, client-side LB) |
